...
const int ledPin = 2;
const uint16_t infraredPin = 14    ;  // ESP8266 GPIO pin to use. Recommended: 4 (D2).
...
EspMQTTClient client(
  "WIFI NAME HERE",
  "WIFI PASSWORD HERE",
  "MQTT SERVER IP HERE",     // MQTT Broker IP
  "MQTT USERNAME HERE",                   // Can be omitted if not needed
  "MQTT PASSWORD HERE",                      // Can be omitted if not needed
  "MQTT CLIENT NAME HERE",               // Client name that uniquely identify your device - this MUST not be the same as any other device!!!
...
void setup()
{
  ...
  client.enableLastWillMessage("office/virtualwall/state", "offline");  // You can activate the retain flag by setting the third parameter to true
...
void onConnectionEstablished()
{
  ...
  client.subscribe("office/virtualwall/command", onCommandReceived);

  // Publish that device is online
  client.publish("office/virtualwall/state", "online"); // You can activate the retain flag by setting the third parameter to true

  // Report the current state of pin to MQTT after connection
  if (char(infraredSwitchNumber) == char('1')) {
    client.publish("office/virtualwall/state/power", "ON");
  }
  else {
    client.publish("office/virtualwall/state/power", "OFF");
  }
}

void onCommandReceived(const String& message) {
  Serial.print("message received from office/virtualwall/command: " + message);
  client.publish("office/virtualwall/state/power", message);
